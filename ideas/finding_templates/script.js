...
function selectTestCase(programName, domain, testCaseId) { readAllProgramData().then((allProgramData) => {
    ...
    for (taskId in testCaseData["tasks"]) {
        ...
        taskDone = task["done"];
        taskFinding = task["isFinding"];
        taskFindingTemplate = task["findingTemplate"]; // todo: add transformations (fill in url in location)
        ...
        taskDiv.innerHTML = `
            ...
            <label style="display: flex; align-items: center; gap: 10px; margin-left: 20px; margin-bottom: 20px;">
            <input type="checkbox" id="checkbox-finding-${taskId}" style="transform: scale(1.2); cursor: pointer;">
            <span style="font-weight: 500;">Mark as finding</span>
            <button id="button-finding-${taskId}" style="
                padding: 5px 10px; 
        ...
        // finding checkbox
        document.getElementById(`checkbox-finding-${taskId}`).checked = taskFinding;
        document.getElementById(`checkbox-finding-${taskId}`).addEventListener('change', closureUpdateFindingHandler(programName, domain, testCaseId, taskId));
        document.getElementById(`button-finding-${taskId}`).addEventListener('click', function() {copyToClipboard(taskFindingTemplate);});
...
function closureUpdateStatusHandler(programName, domain, testCaseId, taskId) { 
    return function() {
        readAllProgramData().then((allProgramData) => {
            ...
            writeAllProgramData(allProgramData);
            // highlight finding
            document.getElementById(`test-case-${testCaseId}`).style.color = this.checked ? "red" : window.getComputedStyle(document.body).color;
        });
    }
}

function closureUpdateFindingHandler(programName, domain, testCaseId, taskId) {
    return function() {
        readAllProgramData().then((allProgramData) => {
            allProgramData[programName]["domains"][domain]["testcases"][testCaseId]["tasks"][taskId]["isFinding"] = this.checked;
            writeAllProgramData(allProgramData);
        });
    }
}
